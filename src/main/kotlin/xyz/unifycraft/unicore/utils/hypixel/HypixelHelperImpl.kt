...
class HypixelHelperImpl : HypixelHelper {
    private val config = HypixelConfig()
    private var apiKey: String
        get() = config.apiKey
        set(value) {
            config.apiKey = value
            UniCore.getConfig().markDirty()
            UniCore.getConfig().save()
        }
    ...
    init {
        ...
        if (config.apiKey.isNotEmpty())
            hypixelApi = HypixelAPI(ReactorHttpClient(UUID.fromString(apiKey)))
    }
    ...
    fun onChatReceived(event: ClientChatReceivedEvent) {
        if (isHypixel()  && config.saveHypixelApiKeys) {
            ...
            Multithreading.schedule({
                UniCore.getHttpRequester().request(Request.Builder()
                    ...
                    response.body?.use { body ->
                        ...
                        this.apiKey = apiKey
                        this.hypixelApi = HypixelAPI(ReactorHttpClient(UUID.fromString(apiKey)))
