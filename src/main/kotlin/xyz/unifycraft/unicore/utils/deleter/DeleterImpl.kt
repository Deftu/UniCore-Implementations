package xyz.unifycraft.unicore.utils.deleter
...
class DeleterImpl : Deleter {
    ...
    fun initialize() {
        val dataDir = UniCore.getFileHelper().dataDir
        file = dataDir.listFiles { dir, name ->
            name.contains("deleter", true)
        }?.firstOrNull() ?: File(dataDir, "Deleter.jar")
    ...
    override fun delete(files: List<File>) {
    ...
    override fun delete(file: File) = delete(listOf(file))
}
