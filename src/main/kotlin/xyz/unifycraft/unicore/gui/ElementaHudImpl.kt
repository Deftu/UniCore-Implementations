...
class ElementaHudImpl : ElementaHud {
    ...
    @Subscribe
    fun onRenderTick(event: RenderTickEvent) { window.draw(UMatrixStack()) }
    @Subscribe
    fun onMouseScroll(event: MouseScrollEvent) { window.mouseScroll(event.delta) }
    @Subscribe
    fun onMouseMove(event: MouseMoveEvent) { window.mouseMove(window) }
    @Subscribe
    fun onMouseClick(event: MouseButtonEvent) { if (!event.released) window.mouseClick(event.x, event.y, event.button) else window.mouseRelease() }
    @Subscribe
    fun onKeyboardInput(event: KeyboardInputEvent) { window.keyType(event.typedChar, event.keyCode) }
}
